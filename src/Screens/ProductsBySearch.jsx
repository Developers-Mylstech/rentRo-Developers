...
function ProductBySearch() {
  const { name } = useParams(); // Changed from brandId to name
    ...
    return '--';
      ...
      const fetchProducts = async () => {
        try {
          ...
          // Updated endpoint to search by name
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!loading && products?.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">We couldn't find any products matching "{name}"</p>
          </div>
        )}
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      <span className="text-blue-600 font-medium text-lg">
                        {product.productFor?.sell?.discountPrice ? `${getProductPrice(product)} AED` : "Request Quotation"}
                      </span>
