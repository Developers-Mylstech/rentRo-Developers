  ...
  return (
    <header
      className={`fixed left-0 right-0 z-50 w-full rounded-b-xl transition-all duration-500 ${
        scrolling
          ? "bg-gradient-to-t from-[#0a448b] to-[#0e86bdcf] bg-opacity-80 shadow-md"
        ...
        <nav className="hidden lg:flex space-x-6 flex-grow justify-center">
          <ul className="flex space-x-6">
            {["rent", "sale", "services", "contact"].map((item) => (
              <li
                key={item}
                className="nav-item relative flex items-center justify-center cursor-pointer transition-all duration-300"
              >
                <Link
                  to={`/${item}`}
                  className={`text-lg font-semibold transition-all duration-300 bg-clip-text text-transparent ${
                    scrolling ? "bg-white" : "bg-white"
                  } hover:animate-[bg-scroll_2s_linear_infinite] px-4 py-2`}
                >
                  {item.toUpperCase()}
                </Link>
              </li>
          ...
          <div className="relative flex items-center">
            <div
              className={`flex items-center relative bg-white border rounded-full transition-all duration-500 overflow-hidden ${
                searchVisible
                  ? "w-36 md:w-64 py-1 pl-4 pr-8 opacity-100 scale-100"
      ...
      >
        <Link to="/login">
          <FiUser
            ...
            className="absolute top-4 left-4 text-3xl cursor-pointer transition-transform duration-200 hover:scale-110 text-white"
          />
        </Link>
        {/* Close Button */}
        <button
